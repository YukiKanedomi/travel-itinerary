...
var PREP_TODO = [
  ...
  { grp:'2〜3ヶ月前', text:'人気店の混雑対策を確認（Chin Chin・Cumulusはウォークイン。17:30〜17:45の早め入りで並びを短く）' },
  ...
  { grp:'1ヶ月前', text:'N\'EXの指定席を2人分購入（えきねっと。台風対策で25号 12:08発に変更済み）' },
  { grp:'1週間前', text:'試しパッキングして荷物はかりで計量（受託23kg・国内線の機内持込8kg）' },
  { grp:'1週間前', text:'お土産リスト（誰に何を・予算）を夫婦で決める → Haigh\'s／QVマーケット／Coles／QVBのどこで買うか当てはめる' },
  { grp:'前日', text:'Visit Japan Web に2人分登録して税関申告のQRを作る（帰国時の羽田用。機内でも可）' }
];
...
var PREP_KB = [
  ...
  { title:'TRS（免税還付）', body:'同一店(同一ABN)で <b>合計AUD$300以上</b>・出発60日以内の購入が対象。GST分（税込価格の1/11・約9%）が戻る。タックスインボイス原本＋商品を手荷物で。<b>出発空港のTRSカウンターで出発30分前まで</b>。TRSアプリで事前入力→QRで時短。' },
  { title:'市内交通', body:'<b>シドニー：Opalカード不要</b>。タッチ決済(クレカ/Apple Pay等)で電車・バス・フェリー・ライトレール全部OK（降車タップ忘れ＝最大運賃に注意）。<b>メルボルン：CBDは無料トラムゾーン</b>（カード不要）。ゾーン外・電車は<b>クレカのタッチ決済でも可</b>（大人運賃・乗降とも同じカード）。<b>旅行中は州の運賃半額キャンペーン中</b>（詳細は情報タブ）。' },
  ...
  { title:'お土産メモ', body:'<b>買えないもの</b>＝カンガルー／エミューのジャーキー、ビーフジャーキー、肉エキス入り食品。日本は肉・肉製品の持ち込みが原則禁止で、<b>真空パックでも空港の免税店で買ったものでも没収</b>。種・植物もNG（ネイティブスパイスは粉になっているものを選ぶ）。蜂蜜は問題なし。<br><b>おもしろい定番</b>＝チキンソルト（鶏は入っていないポテト用の調味料）／ドロップベア（架空の凶暴コアラ）のグッズ／ビンチキン（ゴミを漁る白いトキ。植物園やハイドパークに実物がいる）のグッズ／ベジマイト味のShapes／ネイティブスパイス（レモンマートル・ワトルシード）／ティータオル／Bosisto\'sのユーカリオイル（薬局）／ポリマー紙幣の余り。<br><b>どこで買うか</b>＝DAY2ブロックアーケードの<b>Haigh\'s</b>（1915年創業・豪州最古の家族経営チョコレート店）／DAY3 QVマーケットでスパイスとマカダミア／<b>リッジス地下のColes</b>がスーパー系は最安／DAY6 QVBでT2・R.M.Williams・Oroton（<b>AesopはQVB内になし</b>。188 Pitt Stの店へ）。<b>TRSは同一店$300以上</b>なのでQVBの一店にまとめる。<br><b>アボリジナルアート</b>は模造品が多い。<b>Indigenous Art Codeのロゴ</b>がある店で買い、$250超なら真贋証明書が付くのが正常。大幅な値引きを持ちかける店は避ける。' },
  { title:'受託手荷物とロストバゲージ対策', body:'JAL国際線はエコノミー23kg×2個。<b>豪州国内線VA859は受託1個23kgをeチケットで確認済み</b>（機内持込は8kgのバッグ1個＋座席下の小物1個）。<b>モバイルバッテリーは1人2個まで・機内では充電も給電も不可</b>（JAL 2026/4/24〜。ヴァージンも同様）。対策は<b>夫婦で服を半分ずつ交換して詰める</b>ことと、AirTagの携行。<b>受け取り時にその場で破損を確認</b>（持ち帰ってからでは申告できない）。到着ごとに古い紙タグを剥がすと誤搭載を防げる。' }
];
...
var PACKING = [
  { cat:'貴重品・書類', items:[
    ...
    'ゴールドカードの現物（IASSラウンジ入場用。アプリ画面では入れない）','SkyBusのQRバウチャー（PDF＋スクショ）',
    '家の鍵（1本だけ持つ）' ] },
  { cat:'電子機器', items:[
    ...
    'カメラ（＋予備バッテリー・SDカード）',
    'Apple Watch＋充電器（ケーブル型なら多口USBに挿す）','iPad＋充電器（機内持込。スーツケースに入れない）' ] },
  { cat:'衣類（9月＝春・寒暖差大）', items:[
    ...
    '折りたたみスリッパ（機内・室内用）',
    'サングラス（UVが強い。マグパイ対策にも）','水着（DAY3 インターコンチのプール用・任意）',
    '圧縮袋（帰りの増量にも）','スーツケースベルト・ネームタグ（夫婦で色違い）',
    'マスク数枚（機内の乾燥・混雑用）','ハンカチ・手拭き（紙タオルのないトイレ用）' ] },
  { cat:'衛生・健康', items:[
    ...
    '着圧ソックス（機内用・就寝用は別物）',
    'ロキソニンの英語メモ（薬と同じ袋に）','酔い止め（DAY5 ブルーマウンテンズの山道バス用）',
    '目薬（機内の乾燥）','旅行用洗剤の小分け＋携帯ハンガーか洗濯ばさみ（下着の手洗い用）' ] },
  { cat:'あると便利', items:[
    ...
    'バッグハンガー Heroclip Mini（リュックの持ち手に付けたまま。機内は前席テーブルの留め具に軽い物だけ）',
    '持ち込む食品（FD味噌汁・お菓子）は1袋にまとめる。入国カードは食品YES','割り箸・スプーン（FD味噌汁用）',
    'スーツケースのレインカバーか大きめのゴミ袋（出発日は雨）' ] }
];
...
function renderPrepPage() {
  ...
  /* ToDo */
  h += jmpHTML([['jp-td','ToDo'],['jp-bk','予約'],['jp-pk','持ち物'],['jp-dc','書類'],['jp-kb','基礎知識']]);
  ...
  /* 書類ポケット（中身は docs.js が IndexedDB から描く） */
  h += '<div class="sec-h jmp-t" id="jp-dc">— 書類ポケット（この端末の中だけ） —</div><div id="docs-host"></div>';
