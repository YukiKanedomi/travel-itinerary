...
- **準備**（`renderPrep`）：3つのデータ配列から生成
  - `PREP_TODO`＝出発前ToDo（`grp` でカウントダウン別にグループ化）。チェック状態 localStorage `prep_todo_v1`
  - `PREP_BOOK`＝予約・手配トラッカー。チェック `prep_book_v1`、予約番号メモ `prep_booknote_v1`
  - `PREP_INFO`＝オーストラリア入国・現地の基礎知識カード（ETA/検疫/TRS免税/交通/支払い/電源/水・気候/緊急）。`body` に `<b>` で強調可
  - 持ち物チェックリスト（`#packing-card`・`checklist_v1`）はカテゴリ分けで基本情報から準備タブへ移設
...
AUD⇄円の換算機（info タブ）は、レートを**オンライン自動取得**します。

- **取得元**：`open.er-api.com/v6/latest/AUD`（APIキー不要・CORS可・日次更新）の `rates.JPY`
- **動作**（`initFx`/`fxFetch`/`fxRender`）：起動時にキャッシュ（`localStorage` `fx_rate_v1`＝`{rate, ts}`）を即表示 → 裏でオンライン最新を取得して上書き。失敗時はキャッシュ、無ければ既定 **97円**（`FX_DEFAULT`）
- **更新**：🔄ボタン（`refreshFx`）で手動再取得
- **状態バッジ**（`#fx-status`）：`🟢ライブ`（今取得）/`📶前回値`（キャッシュ）/`参考値`（既定97）。取得日時も表示
- `convRate` はフル精度を保持し、表示のみ小数1桁に丸める（`fxRound`）
...
- **ブラウザ拡大の抑止**：iOS Safari はピンチでページ全体を拡大する（`maximum-scale` も `touch-action` のピンチ抑止も無視）。地図SVG上だけ `gesturestart`/`gesturechange`/`gestureend` と2本指 `touchmove` を `preventDefault` し、ピンチを地図ズームだけに閉じ込める（1本指の縦スクロールは `touch-action:pan-y` で従来どおり可）
