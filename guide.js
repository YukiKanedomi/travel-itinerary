...
/* ---- 出発日（9/20-21）タイムライン ---- */
var GUIDE_GO = [
  { t:'10:53', h:'橋本駅発 — 京王線 新宿行き（台風対策で前倒し）', d:'橋本始発なので座れる。新宿11:38着・409円',
    tips:[ 'N\'EXの指定席は購入済み（チケットレス・25号 10号車2番A/B）', '京王が止まったら控え：JR横浜線で横浜へ → N\'EX横浜発' ] },
  { t:'12:08', h:'N\'EX 25号 新宿発 → 空港第2ビル', d:'約75分・空港第2ビル 13:23着（終点の成田空港まで乗らない）。乗り遅れても12:38発がある',
    tips:[ '新宿での30分はトイレと飲み物に。昼食は成田で。N\'EXは5・6番線' ] },
  { t:'13:23', h:'成田T2着 → まず3階でWi-Fi受取', d:'「J WiFi & Mobile」カウンター（07:00-21:00）。予約メール提示・約5分',
    steps:[ '混んでいたら「法人会員優先レーン」のメールを係員に見せる',
      'ルーターとモバイルバッテリーは手荷物のカバンへ（預け入れ禁止）' ] },
  { t:'13:45', h:'JALカウンターで荷物を預ける', d:'パスポートを出すだけ。搭乗券はWalletのQRでOK', sv:{ lat:35.77325, lng:140.38759, label:'成田空港 第2ターミナル（出発階の車寄せ）', note:'3階出発階の車寄せ。N\'EX を降りて上がると、この車寄せの内側（ロビー）に出る。JAL のカウンターはロビー中央寄り' },
    tips:[ 'オンラインチェックイン済なら「荷物預けのみ」の列が速い',
      '聞かれるのはほぼ一つ「モバイルバッテリーは預け入れに入っていませんか？」→「入っていません」' ] },
  { t:'14:15', h:'身軽になったら昼食と自由時間（17:00まで）', d:'4階展望デッキ（無料）→ IASSラウンジ（保安検査前のエリア）。最後の日本食は検査後の制限エリアで',
    steps:[ '展望デッキは4階の北と南。9月は6:30-21:00・搭乗券不要',
      'IASS EXECUTIVE LOUNGE 2：ゴールドカードの現物＋当日の搭乗券で2人とも無料（家族カード可）。外からの食べ物の持ち込みは不可',
      '3階にユニクロあり。買い忘れの最終チャンス',
      '両替は保安検査の前後どちらにも窓口あり。豪ドルは現地ATMの方が有利なので、ここでは少額だけで十分',
      '検査後にもう一つ休める場所：NARITA SKY LOUNGE 和（サテライト連絡通路2階・無料）' ],
  ...
  { t:'7:00', h:'SkyBusで市内へ', d:'T2を出て隣のターミナル前の赤い停留所から乗車。全便サザンクロス駅行き・約30分。チケットはKKday購入済み',
...
var GUIDE_SOS = [
  ...
  { title:'出発日に台風が来た（JL773が飛ばない）', lead:'先に動くのは電車。成田には午前中に着いておけば、飛行機の判断は空港で待てる。',
    steps:[ '前日夜と当日朝に3つを見る：気象庁の台風情報／JR東日本の運行情報（成田エクスプレス・総武本線）／JAL国際線の運航状況',
      '電車が危ないと感じたら迷わず前倒し。N\'EXはえきねっとの申込履歴から出発前に列車変更できる。N\'EXが止まったら京成スカイライナー（日暮里から）か、東京駅・新宿駅発のリムジンバス',
      '空港に早く着いたら荷物を預けてIASSラウンジで待つ。搭乗券があれば出発時刻の変更や欠航はJALアプリに通知が来る',
      'JL773が欠航したら：その場のJALカウンターで振替を相談（JL773は火・金・日運航。翌日は羽田発シドニー経由なども候補）。JTB海外デスク +81-50-3362-0660 にも一報',
      '振替で到着が遅れるとき：インターコンチに「到着が遅れる」と一報（無断だと予約が消えることがある）。ブルーマウンテンズ（9/24）とVA859（9/23）はそのまま生かせる' ],
    tips:[ '注意｜欠航の連絡を待って家にいない。空港にいる人から順に振替が決まる', 'JALが「台風の特別対応」を出したら、手数料なしで日付を変えられる。出るのは1〜2日前' ] },
