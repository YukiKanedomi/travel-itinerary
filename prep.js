...
var PREP_TODO = [
  ...
  { grp:'2週間前', text:'World eSIM を2人分インストール（自宅Wi-Fiで。QRは別画面か紙に表示して読む）' },
...
var PREP_KB = [
  ...
  { title:'通信（ルーター＋eSIM＋ローミング）', body:'<b>3系統</b>で二重三重に確保済み。①<b>World eSIM 無制限（2人分・9/20〜9/26）</b>＝主力。日付固定なので、出発前に自宅のWi-Fiでインストールしておく（<b>QRは別の画面か紙に表示して読む</b>。同じ端末では読めない）。②<b>ahamo</b>＝申込不要・追加料金なしでそのまま使える（ドコモ系＝Telstra網なので山で強い）。③<b>グローバルWiFi 無制限</b>＝主にiPad用。<b>受取＝成田T2 3階 JWiFi&Mobileカウンター（07:00-21:00）／返却＝羽田T3 2階到着ロビーの返却BOX（24時間可）</b>。<b>ルーターとモバイルバッテリーは機内持込のみ</b>（預け入れ不可・国内線も同じ）。<b>auのデータローミングはOFF</b>のままにする（意図しない課金を防ぐ）。' },
